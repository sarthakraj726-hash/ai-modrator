...
|---|---|---|---|---|---|---|
| **CHAOS-01** | Redis server killed or connection dropped | Cache, Rate Limiting, PubSub | Non-authoritative fail-closed for locks; fallback to in-memory cache. Primary database unaffected. | `InMemoryRedisFallback` auto-engages; logs warning; health reports `DEGRADED`. | `tests/chaos/test_redis_failure_modes.py` | **PASS** |
